...
class TestEuroSymbol:
    def test_euro_symbol_present_with_no_filter(self, client):
        c, _ = client
        _login(c)
        body = c.get("/profile").data.decode()
        assert "€" in body

    def test_euro_symbol_present_with_this_month_filter(self, client):
        c, _ = client
        _login(c)
        body = c.get(
            f"/profile?date_from={THIS_MONTH_FROM}&date_to={THIS_MONTH_TO}"
    ...
    def test_euro_symbol_present_with_custom_filter(self, client):
        c, _ = client
        _login(c)
        body = c.get(
            f"/profile?date_from={EXPENSE_DATE_OLD}&date_to={EXPENSE_DATE_OLD}"
    ...
    def test_euro_symbol_present_on_empty_period(self, client):
